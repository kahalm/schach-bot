...
# ---------------------------------------------------------------------------
# Bibliothek  (Katalog aus index.txt)
# ---------------------------------------------------------------------------
...
def _auto_tag(title: str, author: str, file_ext: str) -> list[str]:
    ...
    for tag, patterns in _LANGUAGE_TAGS.items():
        # Sprach-Tag "French" nur bei Sprache, nicht bei Eröffnung
        if any(re.search(p, text) for p in patterns):
            tags.append(tag)
    return tags
...
def _extract_title_stem(raw_title: str) -> str:
    ...
    # Trailing " - AuthorName" entfernen (häufiges Muster)
...
def _parse_index_entry(line: str) -> tuple | None:
    ...
    # Pfad-Komponenten (Unix-Pfade aus index.txt)
    parts = line.split('/')
    # Basis: /home/kahalm/schach/Author/[SubDir/]File.ext
    try:
        schach_idx = parts.index('schach')
    except ValueError:
        return None

    remaining = parts[schach_idx + 1:]  # Author, [SubDir...], Filename
    if len(remaining) < 2:
        return None

    # Autor = erstes Verzeichnis nach schach/
    author = remaining[0]
    ...
    # Bracket-Pattern entfernen, aber Jahr merken: [Author, 1988]
    year = None
    ...
    # Autor-Name aus Titel entfernen (wenn am Anfang)
    ...
    # Jahr als Fallback aus Titel suchen
    if not year:
